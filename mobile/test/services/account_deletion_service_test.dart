...
void main() {
  group('AccountDeletionService', () {
    ...
    Event createTestEvent({
      ...
      final event = Event(pubkey, kind, tags, content, createdAt: DateTime.now().millisecondsSinceEpoch ~/ 1000);
    ...
    setUp(() {
      ...
      service = AccountDeletionService(nostrService: mockNostrService, authService: mockAuthService);
    ...
    test('createNip62Event should create kind 62 event', () async {
      ...
      when(
        mockAuthService.createAndSignEvent(kind: anyNamed('kind'), content: anyNamed('content'), tags: anyNamed('tags')),
      ).thenAnswer((_) async => expectedEvent);

      // Act
      final event = await service.createNip62Event(reason: 'User requested account deletion');

      // Assert
      expect(event, isNotNull);

      // Verify createAndSignEvent was called with kind 62
      verify(mockAuthService.createAndSignEvent(kind: 62, content: anyNamed('content'), tags: anyNamed('tags'))).called(1);
    });
    ...
    test('createNip62Event should include ALL_RELAYS tag', () async {
      ...
      final captured = verify(
        mockAuthService.createAndSignEvent(kind: anyNamed('kind'), content: anyNamed('content'), tags: captureAnyNamed('tags')),
      ).captured;

      final tags = captured.first as List<List<String>>;
      expect(tags.any((tag) => tag.length == 2 && tag[0] == 'relay' && tag[1] == 'ALL_RELAYS'), isTrue);
    });
    ...
    test('deleteAccount should broadcast NIP-62 event', () async {
      ...
      when(mockNostrService.publishEvent(any)).thenAnswer((_) async => expectedEvent);
    ...
    test('deleteAccount should fail when createAndSignEvent returns null', () async {
      // Arrange
      when(
        mockAuthService.createAndSignEvent(kind: anyNamed('kind'), content: anyNamed('content'), tags: anyNamed('tags')),
    ...
    group('NIP-09 batch deletion', () {
      test('should fetch all user events before deletion', () async {
        ...
        when(mockNostrService.publishEvent(any)).thenAnswer((_) async => nip62Event);
      ...
      test('should publish kind 5 events for user events before NIP-62', () async {
        // Arrange
        final userVideoEvent = createTestEvent(pubkey: testPublicKey, kind: 34236, tags: [], content: 'test video', id: 'video_event_1');
        ...
        when(mockNostrService.queryEvents(any)).thenAnswer((_) async => [userVideoEvent]);

        var createCallCount = 0;
        when(mockAuthService.createAndSignEvent(kind: anyNamed('kind'), content: anyNamed('content'), tags: anyNamed('tags'))).thenAnswer((
          _,
        ) async {
          createCallCount++;
      ...
      test('should group events by kind for batch deletion', () async {
        // Arrange
        final videoEvent1 = createTestEvent(pubkey: testPublicKey, kind: 34236, tags: [], content: 'video 1', id: 'video_1');
        final videoEvent2 = createTestEvent(pubkey: testPublicKey, kind: 34236, tags: [], content: 'video 2', id: 'video_2');
        final likeEvent = createTestEvent(pubkey: testPublicKey, kind: 7, tags: [], content: '+', id: 'like_1');
        ...
        when(mockNostrService.queryEvents(any)).thenAnswer((_) async => [videoEvent1, videoEvent2, likeEvent]);
      ...
      test('should return deletedEventsCount in result', () async {
        // Arrange
        final userEvent = createTestEvent(pubkey: testPublicKey, kind: 1, tags: [], content: 'note', id: 'note_1');
        ...
        when(mockNostrService.queryEvents(any)).thenAnswer((_) async => [userEvent]);
