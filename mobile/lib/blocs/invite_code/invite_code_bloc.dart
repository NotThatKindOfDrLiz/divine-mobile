// ABOUTME: BLoC for invite code entry and claiming
// ABOUTME: Manages invite code validation with synchronous getters for router
...
class InviteCodeBloc extends Bloc<InviteCodeEvent, InviteCodeState> {
  InviteCodeBloc({
    ...
    required InviteCodeRepository repository,
       ...
       super(InviteCodeState(hasClaimedCode: repository.hasClaimedCode)) {
  ...
  bool get hasClaimedCode => _repository.hasClaimedCode;
  ...
  Future<void> _onClaimRequested(
    ...
    try {
      ...
      if (result.valid) {
        await _repository.setClaimedCode(event.code);
        ...
        emit(
          state.copyWith(
            ...
            result: result,
            error: result.message ?? 'Code not found. Please try again.',
          ),
        );
      }
  ...
  void _onReset(InviteCodeReset event, Emitter<InviteCodeState> emit) {
    emit(InviteCodeState(hasClaimedCode: _repository.hasClaimedCode));
  }
}
